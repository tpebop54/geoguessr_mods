...
/**
CREDIT WHERE CREDIT IS DUE// ==UserScript==
// @name         Tpebop's Geoguessr MultiMod V1
// @description  Various mods to make the game interesting in various ways
// @version      1.0
// @author       tpebop
...
const isInBounds = (loc, bounds) => {
    ...
    const south = to360(bounds.south);
    const north = to360(bounds.north);
    const west = to360(bounds.north);
    const east = to360(bounds.east);

    const lat = to360(loc.lat);
    const lng = to360(loc.lng);
...
const updateInFrame = (forceState = null) => {
    ...
    if (active) {
        const showInFrame = () => {
            ...
            const inFrame = isInBounds(currentCenter, currentBounds);
            const color = inFrame ? 'green' : 'red';

            const smallMapStyle = {
                'box-shadow': `0 0 10px 10px ${color} inset`,
                'border-radius': '10px',
            };
            Object.assign(smallMap.style, smallMapStyle);
        };
